...
const PaymentsInformationScreen = () => {
    ...
    const [paymentData, setPaymentData] = useState([
        { id: "1", customer: "John Doe", date: "2025-02-01", amount: "$200", status: "Paid" },
        { id: "2", customer: "Jane Smith", date: "2025-02-05", amount: "$350", status: "Pending" }
    ]);
    ...
    const handleSearch = (query) => {
        ...
        setFilteredPayments(
            paymentApiData.filter(payment => payment.customer.toLowerCase().includes(query.toLowerCase()))
        );
    };
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('https://d90c-2405-201-4025-9105-c8fd-6228-b972-1d8d.ngrok-free.app/api/v1/payments'); // Use the correct API endpoint
                if (!response.ok) {
    ...
    return (
        ...
        >
            <Card style={{ padding: 25, borderRadius: 20, elevation: 5, backgroundColor: "#FFFFFF" }}>
                <Text style={{ fontSize: 22, fontWeight: "bold", color: "#4CAF50", marginBottom: 15, textAlign: "center" }}>💳 Payments Information</Text>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={{ flexDirection: "row", alignItems: "center", padding: 15, borderBottomWidth: 1, borderBottomColor: "#ddd" }}>
                            <FontAwesome name="user" size={24} color="#4CAF50" style={{ marginRight: 10 }} />
                            <View>
                                <Text style={{ fontSize: 18, fontWeight: "bold", color: "#333" }}>Customer: {item?.createdBy}</Text>
                                <Text style={{ fontSize: 16, color: "#666" }}>Date:  {item?.createdAt ? format(new Date(item.createdAt), "dd MMM yyyy, hh:mm a") : "N/A"}</Text>
                                <Text style={{ fontSize: 16, color: "#666" }}>Amount: {item.amount}</Text>
